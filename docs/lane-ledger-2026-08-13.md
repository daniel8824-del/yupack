...
## 심야 — 최대치 검증 라운드 1·2 (오너 지시: "반복되는 검증으로 최대치의 플러그인")
- R1 강건성 배터리(test_plugin_hardening.py 6종): zip-slip 안전 실측(탈출 0)·깨진 zip·경로 순회·비팩 폴더 가드(신규)·질의 퍼징·설정 오염 재인터뷰 + 이어편집 전체 왕복.
- R1 적발·수리 5건: ① 문자열 locator 왕복 소실(라이터 frontmatter 왕복) ② 범위 locator 버퍼 미탑재(hydrate 병합) ③ "- 근거:" 링크를 로더가 미편입 → evidence_refs 정식 편입(4층 리뷰 공백 해소, 기존 팩 소급) ④ 검수 질문 왕복 소실(quality/checks.json 동봉·복원) ⑤ 재저장 위치 포크(hydrate가 원 서가 기억 → 제자리 저장+덮어쓰기 게이트 정상화).
- R2 함대 스위프(배포본 10팩+오디세이): discovery 10/10·프로브 10/10·삼국지 4,353노드 열기 0.7s/질의 0.01s. 적발: 계약 위반 삼국지 600·일리아스 162 → 근본 원인 = 로더 stem2id에서 선행 노드 라벨 폴백이 후행 노드 파일명 스템 선점(동라벨 Event/Evidence 쌍 300엣지 오귀속). 2패스 등록(스템 절대 우선)으로 수리 — 노트 무결이라 기존 배포본 소급 치유(재이행 불요). 재스위프 위반 잔존 0.
- 94/94 GREEN. 앤·일리아스 조작 판정 2건은 기왕 인계된 r2 선언 불일치가 이행본에 승계된 것(일관).

## 심야 2 — 최대치 검증 라운드 3 (동시성·MOC·ingest)
- ingest 경로: z.read 전용(추출 없음) — zip-slip 무해 확인.
- MOC 무결성: 삼국지·일리아스 0건. 오디세이 2건 중 원문 링크는 볼트 실존(정상), `[[대상]]`은 구판 범례 잔재 → 실물 수정 (현행 라이터 템플릿은 무결).
- 동시성 크래시 적발·수리: 노트팩 인메모리 FTS가 스레드 공유 불가(SQLite check_same_thread) → 커넥션 공유 허용 + 잠금 직렬화. 3팩×20질의 병렬 해머 오류 0.
- 94/94 GREEN.
